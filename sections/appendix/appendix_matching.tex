...
Exact computation of the hard-permutation \gls{fgw} cost $c_{\mathrm{FGW}}$ requires a combinatorial search over
...
is sufficient when $\pi_0$ is far from $\pi_{\mathrm{data}}$, so we do not require fine-grained alignment. $\alpha_{*}$ are chosen to balance each contribution equally.

\paragraph{Node matching for molecule-like sources.}
If the source distribution already produces valid molecules, we use a node-matching permutation alignment
based on node labels, followed by computing the
