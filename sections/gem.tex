...
\subsection{Discrete Transport-Aligned Proposal  ($\epsilon=0$)}{\color{red} TODO: point to details}

\label{proposals}
Motivated by the JKO formulation in \Cref{jko_em}, we now derive two discrete kernels corresponding to two distinct sampling regimes, i.e., the transport-dominated regime ($\epsilon=0$) and the mixing regime ($\epsilon>0$).

We assume the energy function takes the form $V_\theta(x) := \tilde{V}_\theta(\hat{x})$, differentiable in the continuous embedding ($\hat{x}$) provided by a neural network parameterization. Similarly, we assume the cost is given by $c(x,y):=\tilde{c}(\hat{x},\hat{y})$, differentiable in both arguments. Starting from a discrete probability distribution $\pi_k$, 

Given an optimal coupling $\gamma$ (in practice approximated using tractable methods, e.g.,~\cite{albergo2024stochastic,tong2024improving}) and its induced transport map $T$, we assume $T$ admits a consistent representation of the form $y=T(x) \iff \hat{y}=\tilde{T}(\hat{x})$, where $\tilde{T}$ is a continuous map that realizes $T$ under the embedding. The deterministic proposal is then defined as the delta measure $\delta_{T(x)}(y)$. Expressing the JKO variational objective from \Cref{jko_em} as an optimization over transport maps (see \Cref{app:jko}) and substituting the time-step parameter $\Delta t$ with the jump step-size $\eta$ yields:
\begin{equation}
\label{eq:new_T}
    T(x)\in\arg\min_{y\sim\pi_{k+1}}\left\{\frac{1}{2\eta}c(x,y)+V_\theta(y)\right\},
\end{equation}
where $\pi_{k+1}=T_{\#}\pi_k$ denotes the pushforward of $\pi_k$ under $T$.
...
We are now, at sampling, trying to solve \Cref{eq:new_T} given a trained network on the above objective. We want to make the proposal local as to iterate the descent in the potential in more than one-shot such that we can also do a taylor expension on $V(y)$ around x. Rather than controlling the step-size parameter $\eta$, we simplify the (local) greedy proposal by restricting the kernel to at most $N$ edits per jump. For this proposal we consider the cost to be $c_{\mathrm{loc}}(x,y)$ with $\lambda_\mathcal{V}=\lambda_\mathcal{E}$ which yields fixed costs for every such candidate - which means the cost is constant and can be neglected within the search. Linearizing $V_\theta(y)\approx V_\theta(x)+(y - x)^\top\nabla V_\theta(x)$ around $x$, we deterministically select the top-$N$ candidate edits ranked by steepest descent. Formally, the candidate set $C_N(x)$ is defined as the set of all edits achievable by exactly $N$ discrete modifications (changing/removing edges or changing nodes) starting from state $x$. The greedy  proposal is thus given by:
\begin{gather}
y^*(x) = x + \arg\min_{\Delta x \in C_N(x)} \Delta x^\top\nabla V_\theta(x), \\
