...
We assume the energy function takes the form $V_\theta(x) := \tilde{V}_\theta(\hat{x})$, differentiable in the continuous embedding ($\hat{x}$) provided by a neural network parameterization. Similarly, we assume the cost is given by $c(x,y):=\tilde{c}(\hat{x},\hat{y})$, differentiable in both arguments. 
