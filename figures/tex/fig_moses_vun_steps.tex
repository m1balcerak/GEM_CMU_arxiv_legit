...
\begin{tikzpicture}[scale=0.95]
  ...
  \node[rotate=90] at (-0.95,2.0) {\glsentryshort{vun}};
  \draw[->, thick] (6.2,0) -- (6.2,4.0);
  \node[rotate=90] at (7.1,2.0) {\glsentryshort{fcd}};
  ...
  \end{scope}
\end{tikzpicture}
\caption{\footnotesize \textbf{\acrshort{vun} and \acrshort{fcd} vs Steps.} \acrshort{vun} (higher is better) and \acrshort{fcd} (lower is better) versus inference steps on MOSES for \gls{gem} noise initialization (uniform) with greedy warmup proposal, \gls{gem} data initialization with annealed proposal, and DeFoG (marginal noise).}
