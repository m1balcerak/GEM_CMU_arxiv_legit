...
\end{tikzpicture}
\caption{\footnotesize \textbf{Validity Along Geodesics.} Average chemical validity (\%) vs.\ distance (node-matching, $c_{\mathrm{FGW}}$) for \gls{gem} energy-weighted versus cost-only geodesics. Cost-only ignores the learned data geometry and interpolates through low-density regions, yielding invalid valences.}
